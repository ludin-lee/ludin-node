...
- **`ludin lint` + health score** — `npx ludin lint spec.yaml [--min 80] [--json]` checks the document (missing summaries, operationIds, descriptions, untagged operations, bodies and responses without schemas, no 2xx, no servers) and prints a health score: the percentage of checks passing, stable across spec sizes. The same result is served at `GET /api/lint` on the filtered document, and the overview screen shows it as a scorecard with the issue list a click away.
