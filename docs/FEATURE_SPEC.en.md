...
- A session-cookie API on **another origin** (docs on localhost, API on api.example.com) leaves the browser nothing to lend, so ludin keeps a jar instead — cookie chaining. Call the login operation from Try it out: the cookies the response sets are kept in the browser tab, per origin, and sent along with every later call to that origin. Since ludin makes the call, the origin does not matter. A `Set-Cookie` that expires a cookie removes it from the jar. It sits under the same switch as token capture, with a *Clear* button, and the audit log masks `cookie` so no value is recorded
- **Pinned headers**: headers every request must carry — an API key the spec never declared, a tenant id, a feature flag — are written once at the top of Try it out and go with every request of every operation. One switch pauses them without losing the values. Headers only: a body or query means something different per operation, so it cannot be global. When a name clashes with an operation's own extra header, the operation wins. Stored in the browser's localStorage; the server is not involved
- **Variables**: write `{{name}}` anywhere in a request — a path or query value, a header, the body, a pinned header, an auth field — and the active environment fills it in. Environments are named sets (`default`, `staging`, `prod` …) switched from one dropdown, so the same request runs against another stack by changing one thing. An unknown name is left as typed. Stored in the browser; the server never sees the variables, only the resolved request
