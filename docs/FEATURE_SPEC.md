...
- 세션 쿠키로 인증하는 API(문서 옆에 사는 `/admin`·`/console`)도 Try it out 가능: `forwardCookies`를 켜면 프록시가 호출자 **본인의** 쿠키를 대상에 넘긴다. 문서와 **같은 origin**일 때만 — 다른 origin의 쿠키는 브라우저가 루딘에 보내지 않으므로 전달할 것 자체가 없다. 루딘 자체 세션·공유 쿠키는 항상 제외되며, 이름 목록으로 좁힐 수 있다. 기본 off. 응답의 `Set-Cookie`는 데이터로 돌아올 뿐 브라우저에 반영되지 않으므로 대상에 먼저 로그인해 둔 상태여야 한다
- **다른 origin**의 세션 쿠키 API(문서는 localhost, API는 api.example.com)는 브라우저가 빌려줄 쿠키가 없으므로 루딘이 대신 보관한다 — 쿠키 체이닝. Try it out으로 로그인 오퍼레이션을 호출하면 응답이 설정한 쿠키를 브라우저 탭 안에 origin별로 보관하고, 그 origin으로 가는 이후 호출에 함께 보낸다. 호출 주체가 루딘이므로 origin은 상관없다. 쿠키를 만료시키는 `Set-Cookie`는 보관함에서 지운다. 토큰 자동 수집과 같은 스위치 아래 놓이고 "해제" 버튼이 있으며, 감사 로그에는 `cookie`가 마스킹되어 값이 남지 않는다
- **고정 헤더**: 스펙이 선언하지 않은 API 키, 테넌트 ID, 기능 플래그처럼 모든 요청에 실어야 하는 헤더를 Try it out 상단에서 한 번 적으면 모든 오퍼레이션의 모든 요청에 붙는다. 스위치 하나로 값을 잃지 않고 잠시 끌 수 있다. 헤더만 — 바디·쿼리는 오퍼레이션마다 뜻이 달라 전역으로 둘 수 없다. 오퍼레이션의 추가 헤더와 이름이 겹치면 오퍼레이션 쪽이 이긴다. 브라우저 localStorage에 저장되며 서버는 관여하지 않는다
- **환경 변수**: 요청 어디서든 — 경로·쿼리 값, 헤더, 바디, 고정 헤더, 인증 칸 — `{{이름}}`을 쓰면 활성 환경의 값으로 채워진다. 환경은 이름 붙은 변수 묶음(`default`, `staging`, `prod` …)이고 드롭다운 하나로 바꾸므로 같은 요청을 다른 스택에 그대로 보낼 수 있다. 모르는 이름은 적은 그대로 둔다. 브라우저에 저장되며 서버는 변수를 보지 않고 치환된 요청만 받는다
...
- 이름 있는 프리셋(`theme.preset`): 다크 `graphite`·`nocturne`·`fjord`·`phosphor`·`contrast`, 라이트 `paper`·`blossom`·`cloud`·`mint`. 각 프리셋은 고정된 완전한 색 체계라 라이트·다크·시스템 토글은 `default`에만 적용된다. 사용자 메뉴에서 전환 가능(브라우저별 저장), `primary`·`accent`·`font`·`radius`는 운영자가 지정한 프리셋 위에만 적용되고, 사용자가 다른 프리셋으로 바꾸면 그 프리셋 고유 값을 쓴다(브랜드 색이 맞지 않는 팔레트 위에 올라가지 않도록)
